...
public class DoctorController {
    private final DoctorInputPort doctorInputPort;
     ...
     */
    @Operation(summary = "의료진 회원가입", description = "아이디와 패스워드, 추가 데이터로 회원가입")
    @PostMapping("/v3/doctor/sign-up")
    public ResponseEntity<SignUpResultResponse> signUp(@RequestBody DoctorSignUpRequest body) {
        doctorInputPort.signUp(body.toInputPortDTO());
    ...
    @GetMapping("/v3/doctor/{doctorId}")
    public ResponseEntity<DoctorDataResponse> get(@PathVariable String doctorId) {
        DoctorProfileInputPortResponse profileDTO = doctorInputPort.getDoctorProfile(doctorId);
